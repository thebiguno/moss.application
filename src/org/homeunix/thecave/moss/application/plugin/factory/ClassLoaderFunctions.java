...
//					Log.debug("JarLoader.getAllClasses(): added " + je.getName() + " to collection of jar entries.");
			...
			final Logger logger = Logger.getLogger(ClassLoaderFunctions.class.getName());
			logger.log(Level.WARNING, "Unknown error", ioe);
		}
...
//	}
	...
	private Object getObject(String className) throws JarLoaderException {
		final Logger logger = Logger.getLogger(ClassLoaderFunctions.class.getName());
		try{
			logger.finest("Trying to load object from class " + className);
			...
			logger.log(Level.SEVERE, "Could not instantiate object", iae);
		}
		catch (InstantiationException ie){
			logger.log(Level.SEVERE, "Could not instantiate object", ie);
		}
		catch (NoClassDefFoundError ncdfe){
			logger.log(Level.SEVERE, "Could not instantiate object", ncdfe);
		}
		catch (Exception e){
	...
	public static Class<?> getClass(File jarFile, String className) throws JarLoaderException {
		try {
			String jarFileURL = "jar:file:" + jarFile.getAbsolutePath() + "!/";
			final Logger logger = Logger.getLogger(ClassLoaderFunctions.class.getName());
			logger.finest("Attempting to load from " + jarFileURL);
			URL url = new URL(jarFileURL);
